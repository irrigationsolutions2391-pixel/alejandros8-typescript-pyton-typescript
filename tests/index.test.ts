...
import { APIPromise } from 'alejandros8-typescript-pyton/core/api-promise';

import util from 'node:util';
import Alejandros8TypescriptPyton from 'alejandros8-typescript-pyton';
import { APIUserAbortError } from 'alejandros8-typescript-pyton';
...
describe('instantiate client', () => {
  ...
  describe('defaultHeaders', () => {
    const client = new Alejandros8TypescriptPyton({
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['ALEJANDROS8_TYPESCRIPT_PYTON_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Alejandros8TypescriptPyton) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new Alejandros8TypescriptPyton({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['ALEJANDROS8_TYPESCRIPT_PYTON_LOG'] = 'debug';
      const client = new Alejandros8TypescriptPyton({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['ALEJANDROS8_TYPESCRIPT_PYTON_LOG'] = 'not a log level';
      const client = new Alejandros8TypescriptPyton({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'ALEJANDROS8_TYPESCRIPT_PYTON_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['ALEJANDROS8_TYPESCRIPT_PYTON_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Alejandros8TypescriptPyton({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['ALEJANDROS8_TYPESCRIPT_PYTON_BASE_URL'] = 'https://example.com/from_env';
      const client = new Alejandros8TypescriptPyton({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['ALEJANDROS8_TYPESCRIPT_PYTON_BASE_URL'] = ''; // empty
      const client = new Alejandros8TypescriptPyton({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });

    test('blank env variable', () => {
      process.env['ALEJANDROS8_TYPESCRIPT_PYTON_BASE_URL'] = '  '; // blank
      const client = new Alejandros8TypescriptPyton({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['ALEJANDROS8_TYPESCRIPT_PYTON_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Alejandros8TypescriptPyton({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Alejandros8TypescriptPyton({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Alejandros8TypescriptPyton({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Alejandros8TypescriptPyton({ apiKey: 'My API Key', fetch: testFetch });
